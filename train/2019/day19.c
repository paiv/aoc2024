// gcc -O2 -Wall day19.c -o day19 && ./day19 < day19.in
#include <errno.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static word
probe(struct ivm_image* program, size_t px, size_t py) {
    static word code[0x200000];
    ...
    word chr = 0;
    ivm_pipe_write(pin, px);
    ivm_pipe_write(pin, py);
    for (;;) {
...
explore_grid(char* data, int datasize, char grid[gridsizey][gridsizex]) {
    ...
    for (size_t py = 0; py < 50; ++py) {
        for (size_t px = 0; px < 50; ++px) {
            #if IVM_TRACE > 2
            fprintf(stderr, "%ld,%ld: ", px, py);
            #endif
            word chr = probe(&program, px, py);
            #if IVM_TRACE > 2
            fprintf(stdout, "%lld\n", chr);
            #endif
            grid[py][px] = chr ? '#' : '.';
...
solve2(char* data, int datasize) {
    ...
    for (;; ++px) {
        ...
        if (q == 0) {
            if (px >= 100) {
                q = probe(&program, px - 100, py + 99);
                if (q != 0) {
                    return (px - 100) * 10000 + py;
                }
            }
            px -= 2;
            py += 1;
